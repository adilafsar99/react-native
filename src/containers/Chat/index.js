...
const Chat = (props) => {
  ...
  const send_message = async (event) => {
    if (event.keyCode === 13) {
      ...
      let notificationsCol = collection(db, "notifications");
      let notificationDoc = doc(notificationsCol, chat_id);
      await addDoc(allMessagesRef, {
        message, uid, chat_id, timestamp: new Date()});
      await setDoc(notificationDoc, {message: message});
      setMessage("");
      getAllMessages();
    }
  }
            ...
            <div className="chat_container">
              <div className="header">
                ...
                <div className="username">
                  <h4>{currentChat && currentChat.firstName + " " + currentChat.lastName}</h4>
                </div>
              ...
              <div className="message_box">
                ...
                <div className="icon_box_send">
                  <FiSend onClick={send_message} color="#fff" size={18} />
                </div>
              </div>
            </div>
          </div>
